...
# TODO: Add more specialized handlers here
...
# TODO: Add more handlers for other sites
# Examples of future handlers:
# - SeriousEatsHandler (using their RSS/API)
# - MichelinGuideHandler (using their structured data)
# - ThrillistHandler (using RSS feeds)
# - TimeOutLocalHandler (for specific cities with different URL structures)
...
class EaterTimeoutSpecializedScraper:
    ...
    def _log_stats(self):
        ...
        if self.stats['total_processed'] > 0:
            ...
            logger.info(f"  Success rate: {success_rate:.1f}%")emap_url, url)

            if extraction_result.success:
                self.stats["successful_extractions"] += 1
                self.stats["total_articles_found"] += len(extraction_result.articles)
    ...
    async def _fetch_and_parse_rss(self, rss_url: str, original_url: str) -> SpecializedExtractionResult:
        """Fetch and parse RSS feed to extract restaurant articles"""
        try:
    ...
    async def _fetch_and_parse_sitemap(self, sitemap_url: str, original_url: str) -> SpecializedExtractionResult:
        """Fetch and parse sitemap to extract restaurant articles"""
        try:
